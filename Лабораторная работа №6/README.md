...
```
2. Перегрузить оператор == (оператор равенства). Например, для варианта с фильмами, оператор сравнения может возвращать результат сравнения названия фильмов:
```cpp
...
...
 ...
 ```
 3. Перегрузить оператор потокового вывода << для ofstream внутри вашего класса (игр, фильмов и т.д), для того чтобы можно было записывать данные в файл при помощи следущей записи:
 ```cpp
 ...
 ```
 4. Для того, чтобы в TextBrowser можно было писать так же, как в консоль через cout, перегрузите оператор << для QtextBrowser, чтобы им можно было пользоваться следующим образом:
```cpp
...
2. Что такое бинарный оператор?
3. Какие операторы можно перегрузить?
